...
sealed interface ImageSummaryState {
    ...
    @Immutable data class Success(val labels: List<String>) : ImageSummaryState
 ...
 *
 * ML Kit 摘要在 [init] 階段交給 [PicsumImageAnalyzer]——以縮圖為輸入、結果走
...
class PicsumDetailViewModel(
    ...
    init {
        viewModelScope.launch {
            ...
            val url = photo.thumbnailUrl(MLKitPicsumImageAnalyzer.ANALYZE_THUMB_SIZE)
            analyzer.summarize(photo.id, url)
                .onSuccess { labels ->
                    uiState.update { it.copy(summary = ImageSummaryState.Success(labels)) }
                }
                .onFailure { e ->
                    uiState.update {
